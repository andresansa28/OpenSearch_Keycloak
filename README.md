...
## Prerequisiti vm remota

- Avere ssh.service attivo (eseguirlo con `sudo systemctl start ssh.service`)
	- se non installato, eseguire `sudo apt install ssh`
- aprire il file visudo con il comando `sudo visudo`
	- alla fine del file incollare questa riga: `<nome_utente> ALL=(ALL) NOPASSWD:ALL
`	
- Installare tcpdump ``sudo apt install tcpdump``
